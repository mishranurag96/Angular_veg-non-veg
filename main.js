...
foodieApp.controller('restaurantController', function($scope, $routeParams, $http) {
	...
	var restaurants = [{
		...
		image: 'https://www.incredibleegg.org/wp-content/uploads/basic-french-omelet-930x550.jpg',
		bestDish: {
			name: 'Corn Pizza',
			image: 'https://www.incredibleegg.org/wp-content/uploads/basic-french-omelet-930x550.jpg'
		}
	...
	$scope.getIngredients = function(url) {
		...
		$http({
			...
			for (var i = 0; i < nonVegItems.length; i++) {
				if(ingredients.filter(function(item){ return item.name === nonVegItems[i] && item.value >= 0.75;}).length) {
					$scope.nonVegFood = true;
				}
			}
